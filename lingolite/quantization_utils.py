...
Provides helpers for dynamic/post-training quantization and quantization
aware training (QAT). The functions rely on PyTorch's built-in
...
try:  # PyTorch 1.13+
    ...
    from torch.quantization import (  # type: ignore
        convert,
        get_default_qat_qconfig,
        get_default_qconfig,
        prepare,
...
def apply_dynamic_quantization(
    ...
    modules: Iterable[type] = (torch.nn.Linear,),
) -> torch.nn.Module:
    """Apply dynamic quantization to the specified module types."""
...
def convert_static_quantization(model: torch.nn.Module) -> torch.nn.Module:
    """Convert a prepared model to its quantized representation."""
